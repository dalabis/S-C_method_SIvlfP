...
%коэффициенты, которые учитывают фокусирующее/дефокусирующее действие
%сферических поверхностей земли и ионосферы
%alpha1 - дл€ первого ионосферного луча
%alpha2 - дл€ второго ионосферного луча
alpha1 = ((a + h) / a) *...
    sqrt(2 * 1 * sin(theta / (2 * 1)) * (((a + h) / a) - cos(theta / (2 * 1)))) /...
    sqrt(sin(theta) * (((a + h) / a) * cos(theta / (2 * 1)) - 1));
alpha2 = ((a + h) / a) *...
    sqrt(2 * 2 * sin(theta / (2 * 2)) * (((a + h) / a) - cos(theta / (2 * 2)))) /...
    sqrt(sin(theta) * (((a + h) / a) * cos(theta / (2 * 2)) - 1));
